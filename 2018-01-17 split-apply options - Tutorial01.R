...
#XXX = LAST
#Start from inside and work backward

# this is where the magic happens
lapply(serengeti.Site, dim) 

dim(serengeti.Site$U12)
...
for (i in length(serengeti.Site)) print(dim(serengeti.Site[[i]]))

for (i in length(serengeti)){
	group_by()
...
for (i in seq_along(serengeti.Site)) {
	print(site.dim.for[[i]])
	print(site.dim.lapply[[i]])
}

for (i in 1:length(site.dim.for)) {
	print((site.dim.for[[i]] == site.dim.lapply[[i]]))
}
...
serengeti.Spp <- split(serengeti.behaviours, serengeti$Species)
